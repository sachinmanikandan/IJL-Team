...
export default function Hanchou() {
  ...
  const [HanContents, setHanContents] = useState<HanContent[]>([]);
  ...
  useEffect(() => {
    const fetchHanContents = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://127.0.0.1:8000/han-content/`);

        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data: HanContent[] = await response.json();
        console.log('Han Contents:', data);
  ...
  useEffect(() => {
    const fetchTrainingContents = async () => {
      if (HanContents.length === 0) return;

      const activeTabIndex = parseInt(activeTab.replace('tab', '')) - 1;
      const currentHanContent = HanContents[activeTabIndex];

      if (!currentHanContent) return;

      try {
        // Use the new endpoint with han_content_id
        const response = await fetch(
          `http://127.0.0.1:8000/han-training-content-by-id/${currentHanContent.id}/`
        );
  ...
  }, [activeTab, HanContents]);

  // Generate tabs from subtopic contents
  const detailTabs: TabData[] = HanContents.map((content, index) => ({
    id: `tab${index + 1}`,
    title: content.title,
  ...
  const getCurrentSubcontentId = (): number | null => {
    const activeTabIndex = parseInt(activeTab.replace('tab', '')) - 1;
    const currentHanContent = HanContents[activeTabIndex];
    return currentHanContent ? currentHanContent.id : null;
  };
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('han_content', currentHanContent.id.toString());
      ...
      const response = await fetch('http://127.0.0.1:8000/han-training-content/create/', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDeleteContent = async (contentId: string) => {
    ...
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/han-training-content/${contentId}/`);
