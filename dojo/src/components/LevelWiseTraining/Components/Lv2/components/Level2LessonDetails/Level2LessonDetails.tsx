...
// Updated function to parse subunits into tabs
const parseSubunitsToTabs = (subunits: Subunit[]): TabData[] => {
  const tabs: TabData[] = [];
  
  // Add subunit tabs only
  subunits.forEach((subunit, index) => {
    tabs.push({
      id: `subunit-${subunit.id}`,
      title: subunit.title,
      content: subunit.title === 'MCQ' 
        ? 'Multiple Choice Questions for assessment and evaluation.'
        : `Detailed training content about ${subunit.title} .`,
      isSubunit: true,
...
export default function Level2LessonDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://127.0.0.1:8000/level2-units/${id}/`);
        ...
        // Set first subunit as active tab if available
        if (data.subunits && data.subunits.length > 0) {
          setActiveTab(`subunit-${data.subunits[0].id}`);
        }
      } catch (err) {
  ...
  const fetchTrainingContents = async () => {
    if (!lesson || !activeTab) return;

    const activeSubunitId = activeTab.replace('subunit-', '');
    const currentSubunit = lesson.subunits.find(subunit => subunit.id.toString() === activeSubunitId);

    if (!currentSubunit) return;
    console.log('currentSubunit',currentSubunit)
    try {
      // Use the same endpoint as upload
      const response = await fetch(
        `http://127.0.0.1:8000/subunitwise-training-content/by-subunit/${currentSubunit.id}/`
      );

      if (response.ok) {
        const data: TrainingContent[] = await response.json();
        console.log('Level 2 training contents:', data);
...
const handleUploadSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    formData.append('subunit', currentSubunit.id.toString());
    ...
    const response = await fetch('http://127.0.0.1:8000/level2-contents/', {
      method: 'POST',
      body: formData,
    });

    if (response.ok) {
      // After successful upload, fetch the updated contents
      const fetchResponse = await fetch(
        `http://127.0.0.1:8000/subunitwise-training-content/by-subunit/${currentSubunit.id}/`
      );
      
      if (fetchResponse.ok) {
        const data: TrainingContent[] = await fetchResponse.json();
        setTrainingContents(data);
      } else {
        console.error('Failed to fetch updated contents');
      }
  ...
  const detailTabs = parseSubunitsToTabs(lesson.subunits);
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="w-72 bg-white shadow-md">
        {/* <div className="p-4 border-b border-gray-200">  
          <h2 className="text-xl font-semibold text-gray-800">{lesson.content}</h2>
          <p className="text-sm text-gray-600 mt-1">Day {lesson.day}</p>
          <p className="text-xs text-gray-500 mt-1">Topic {lesson.topic} {lesson.subtopic && `• Subtopic ${lesson.subtopic}`}</p>
        </div> */}
        <nav className="flex flex-col p-2">
          {detailTabs.map((tab, index) => (
            <button
              ...
              className={`p-3 my-1 text-left rounded-lg transition-colors duration-200 flex items-center ${
                activeTab === tab.id ? "bg-blue-500 text-white" : "text-gray-700 hover:bg-blue-100"
              }`}
            >
        ...
        <div className="mx-auto">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-sm text-gray-600">
              Day {lesson.day} | Content {detailTabs.findIndex(tab => tab.id === activeTab) + 1} of {detailTabs.length}
            </div>
          </div>
