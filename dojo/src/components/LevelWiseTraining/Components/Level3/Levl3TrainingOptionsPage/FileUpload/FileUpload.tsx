...
export default function Levle3FileUploadComponent() {
  const location = useLocation();
  const { lineId } = (location.state as LocationState) || {};
  ...
  const fetchTrainingContents = async () => {
    try {
      setLoading(true);
      const url = lineId 
        ? `http://127.0.0.1:8000/levelthree-contents/?topic_id=${lineId}`
        : 'http://127.0.0.1:8000/levelthree-contents/';
  ...
  const handleDeleteContent = async (contentId: string) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/levelthree-contents/${contentId}/`);
