...
const MasterTableSettings = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/upload-operator-excel/`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleOperatorChange = (index: number, field: keyof OperatorMaster, value: string | null) => {
  ...
  const hasErrors = operators.some(operator => 
    !operator.employee_code.trim() || !operator.full_name.trim()
  );

  if (hasErrors) {
    setSubmitError('Please fill all required fields (Employee Code and Full Name)');
  ...
  try {
    ...
    setSubmitError(err instanceof Error ? err.message : 'Failed to submit operators');
  } finally {
    setIsSubmitting(false);
  }
};

  const renderField = (field: FormField, operator: OperatorMaster, index: number) => {
    ...
    if (field.type === 'date') {
      return (
        <input
          ...
          onChange={(e) => handleOperatorChange(index, field.name as keyof OperatorMaster, e.target.value || null)}
        ...
        <div className="space-y-4">
          <div className="flex flex-col space-y-4">
            ...
            <button
              ...
              className={`inline-flex items-center justify-center px-6 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${!file ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {isUploading ? (
                <>
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-green-700">
                    <p>
                      <span className="font-medium">Success:</span> {uploadResult.success_count} records
                    </p>
                    {uploadResult.error_count > 0 && (
                      <p>
                        <span className="font-medium">Errors:</span> {uploadResult.error_count} records
                      </p>
                    )}
                  </div>
                  {uploadResult.errors && uploadResult.errors.length > 0 && (
                    <div className="mt-3 border-t border-green-200 pt-3">
                      <h4 className="text-xs font-medium text-green-800 uppercase tracking-wider">Error details:</h4>
        ...
        <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {fields.map(field => (
                  <div key={field.name} className={`col-span-1`}>
                    ...
                    {renderField(field, operator, index)}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="pt-4">
            <button
              ...
              className={`w-full flex justify-center items-center px-4 py-2 rounded-md text-white ${isSubmitting ? 'bg-blue-300' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
              {isSubmitting ? (
                <>
